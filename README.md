...
Il cleartext (a decrypted keystream) is used by Hive ransomware when encrypting each file. When encrypting a file, Hive ransomware calculates two integers referring to precise positions in the cleartext (offsets) to be used to encrypt the file according to the following formula:
...
However, the first offset is encrypted using a fixed position of the cleartext and different for each Hive 5/5.1/5.2 sample.
...
- 0xFF means that the ransomware calculated a different NCB (not encrypted block) for each file and limited itself to encrypting portions of 0x100000 bytes
...
2. Given a file with a known header (PDF, JPG, PNG, Office file) brute the possible value of the special offset by decrypting the first bytes and looking for a match with the known signature
